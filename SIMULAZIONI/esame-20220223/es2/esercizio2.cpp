...
void inserisci_nodo(List * &l, List *nodo){
	if(l == NULL){
		l = nodo;
		return;
	}
	...
	else inserisci_nodo(l->next, nodo);

}
void compute_list_aux(List *l, List * &s1, List * &s2, int cont1, int cont2){
	if(l == NULL){
		inserisci_nodo(s1, new List{cont1, NULL});
		inserisci_nodo(s2, new List{cont2, NULL});
		return;
	}
	if(l->info % 2 == 0 && l->info % 3 != 0){
		inserisci_nodo(s1, new List{l->info, NULL});
		cont1++;
	}
	else if(l->info % 3 == 0){
		inserisci_nodo(s2, new List{l->info, NULL});
		cont2++;
	}
	compute_list_aux(l->next, s1, s2, cont1, cont2);
