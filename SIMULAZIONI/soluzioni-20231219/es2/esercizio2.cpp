...
void estrai_aux(const tree *t, list * &r, int somma, int cont){
    if(t == nullptr){
        return;
    }
    if(((t->left == nullptr || t->right == nullptr) && t->info % 2 == 0)){
        //è un ramo con valore pari
        somma += t->info;
        cont++;
        std::cout << somma << '/' << cont << std::endl;
        double media = (double)somma/(double)cont;
        //inserisco nella lista
        list *nuovo_nodo = new list{media,r};
        r = nuovo_nodo;
    }else if(t->info % 2 == 0){
        somma += t->info;
        cont++;
    }
    estrai_aux(t->left, r, somma, cont);
    estrai_aux(t->right, r, somma, cont);
}
void calcola_lista_medie(const tree *t, list * &r){
    r = nullptr;
    if(t == nullptr)
