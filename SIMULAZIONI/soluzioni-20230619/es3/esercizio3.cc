...
void converti_aux(Nodo *l, int **mat, int riga, int j){
    if(l != NULL){
        mat[riga][j] = l->valore;
        if(l->figlio != nullptr){
            converti_aux(l->figlio, mat, riga+1, j);
        }
        converti_aux(l->prossimo, mat, riga, j+1);
    }
}
int** convertiListaInMatrice(Nodo* testa, int righe, int colonne) {
    ...
    for(int i = 0; i < righe; i++){
        int *v = new int[colonne];
        for(int j = 0; j < colonne; j++)
            v[j] = -1;
        mat[i] = v;
    }
    converti_aux(testa, mat, 0, 0);
