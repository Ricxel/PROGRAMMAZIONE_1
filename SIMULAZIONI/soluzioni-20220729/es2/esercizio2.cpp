...
int conta_occorrenze(list *l, int code){
  ...
  return somma + conta_occorrenze(l->next, code);
}
...
void estrai_aux(list *l1, list *l2, list * &ris){
  if(l1 == NULL){
    return;
  }
  int value;
  if(l1->code % 2 == 0){
    ...
    value = conta_occorrenze(l2, l1->code);
  }
  else{
    value = -1;
  }
  //inserisco il nodo
  list *nuovo_nodo = new list{value, NULL};
  inserisci_nodo(ris, nuovo_nodo);
  estrai_aux(l1->next, l2, ris);
}
list * estrai(list *l1, list *l2){
  list *ris = NULL;
  estrai_aux(l1,l2,ris);
  return ris;
}
